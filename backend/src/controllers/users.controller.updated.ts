...
export class UsersController {
  ...
  static async create(req: Request, res: Response) {
    try {
      ...
      const sshConfig: SSHConfig = {
        ...
        privateKey: decrypt(vps.privateKey),
      };
      ...
      if (!result.success) {
        await prisma.actionLog.create({
          data: {
            ...
            action: 'create_user',
            status: 'error',
            errorMessage: result.error || result.stderr || 'Failed to create user',
          },
        });
  ...
  static async renew(req: Request, res: Response) {
    try {
      ...
      const sshConfig: SSHConfig = {
        ...
        privateKey: decrypt(user.vps.privateKey),
      };
  ...
  static async delete(req: Request, res: Response) {
    try {
      ...
      if (!result.success) {
        logger.warn(`Failed to delete user on VPS, but removing from DB: ${result.stderr}`);
      }
